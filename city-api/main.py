...
# Kafka configuration
KAFKA_NODES = os.getenv('KAFKA_BOOTSTRAP_SERVERS', 'kafka:9092')
CONTROL_TOPIC = "city-control"
NAMESPACE = os.getenv('KUBERNETES_NAMESPACE', 'global-city-streaming')
...
# Function to get pod logs using Kubernetes API
def get_pod_logs(pod_name, container_name=None, tail_lines=2000):
    try:
        ...
        print(f"Error getting logs for pod {pod_name}: {str(e)}")
        return ""

# Function to find pod by label
def find_pod_by_label(label_selector):
    try:
        ...
        print(f"Error finding pod with label {label_selector}: {str(e)}")
        return None
...
async def proxy_flink_raw_logs():
    ...
    pod_name = find_pod_by_label("app=flink-processor")
    if not pod_name:
        return {"logs": "Flink processor pod not found"}
...
async def get_kafka_logs():
    ...
    try:
        pod_name = find_pod_by_label("app=kafka-producer")
        if not pod_name:
            return {"error": "Kafka producer pod not found"}
            
        logs = get_pod_logs(pod_name)
        print(f"MY KAFKA LOGS: Retrieved {len(logs)} bytes of logs from pod")
